...
function simpleRows(rows, cols){{return '<table><thead><tr>'+cols.map(c=>'<th>'+esc(c)+'</th>').join('')+'</tr></thead><tbody>'+rows.map(o=>'<tr>'+cols.map(c=>'<td>'+((c.toLowerCase().includes('cost')||c=='Cost')?money(o[c]):esc(o[c]))+'</td>').join('')+'</tr>').join('')+'</tbody></table>'}}

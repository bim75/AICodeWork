...
function advisorRow(o,i){{let k=recKey(o,i); return '<tr><td><input type="checkbox" data-key="'+esc(k)+'" '+(selectedRecs.has(k)?'checked':'')+' onchange="toggleRec(this.dataset.key,this.checked)"></td><td class="money">'+money(o._savings)+'</td><td>'+esc(o.currency)+'</td><td>'+esc(o.impact)+'</td><td>'+esc(o.resourceName||o.impactedValue)+'</td><td>'+esc(o.resourceType||o.impactedField)+'</td><td>'+esc(o.resourceGroup)+'</td><td>'+esc(o.subscription)+'</td><td>'+esc(o.resourceId)+'</td></tr>'}}
function advisorTable(){{let rows=filtered(DATA.advisor).map((o,i)=>Object.assign({{_idx:i,_savings:num(o.savings)}},o)); let groups={{}}; rows.forEach((o,i)=>{{let key=val(o.shortDescription)||'Recommendation'; if(!groups[key]) groups[key]=[]; groups[key].push(Object.assign({{_displayIdx:i}},o));}}); let ordered=Object.keys(groups).map(k=>({{key:k,rows:groups[k],savings:groups[k].reduce((a,o)=>a+o._savings,0)}})).sort((a,b)=>b.savings-a.savings || b.rows.length-a.rows.length); let h='<div class="card"><h2>Advisor cost recommendations ('+rows.length+')</h2><p class="small">Grouped by recommendation description. Each group shows the first 10 affected resources; click the + / details row to expand the rest. Checkboxes only update the savings calculator; they do not change Azure.</p>'; ordered.forEach(g=>{{let shown=g.rows.slice(0,10); let hidden=g.rows.slice(10); h+='<details class="card" '+(ordered.length<=3?'open':'')+'><summary><b>'+esc(g.key)+'</b> — '+g.rows.length+' resources — possible savings <span class="money">'+money(g.savings)+'</span></summary><table><thead><tr><th>Use?</th><th>savings</th><th>currency</th><th>impact</th><th>resourceName</th><th>resourceType</th><th>resourceGroup</th><th>subscription</th><th>resourceId</th></tr></thead><tbody>'; h+=shown.map((o,i)=>advisorRow(o,o._displayIdx)).join(''); if(hidden.length){{h+='<tr><td colspan="9"><details><summary>+ show '+hidden.length+' more affected resources</summary><table><tbody>'+hidden.map((o,i)=>advisorRow(o,o._displayIdx)).join('')+'</tbody></table></details></td></tr>';}} h+='</tbody></table></details>';}}); return h+'</div>'}}

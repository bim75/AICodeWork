...
def id_parts(resource_id):
    ...
    if 'providers' in parts:
        ...
        if i+2 < len(parts): rtype=parts[i+1]+'/'+parts[i+2]
        if i+3 < len(parts): rname=parts[-1]
    return rname, rtype, rg

def enrich_cost_row(c, resource_lookup):
    rid=str(c.get('ResourceId') or c.get('resourceId') or '')
    r=resource_lookup.get(rid.lower(),{})
    parsed_name, parsed_type, parsed_rg = id_parts(rid)
    c['ResourceName']=c.get('ResourceName') or r.get('name') or parsed_name
    c['ResourceType']=c.get('ResourceType') or r.get('type') or parsed_type
    c['ResourceGroupName']=c.get('ResourceGroupName') or r.get('resourceGroup') or parsed_rg
    return c

all_resources=[]; all_cost=[]; all_service=[]; all_advisor=[]; findings=[]; all_utilization=[]
for s in subs:
    ...
    r_by_id={str(r.get('id','')).lower():r for r in resources if r.get('id')}
    costs=[enrich_cost_row(c, r_by_id) for c in costs]
    ...
    for r in resources:
        ...
        t=(r.get('type') or '').lower(); p=r.get('properties') or {}; m=m_by_id.get(rid,{})
        if t=='microsoft.compute/virtualmachines':
            hp=(p.get('hardwareProfile') or {}) if isinstance(p,dict) else {}
            all_utilization.append({'category':'VM CPU','subscription':s['name'],'subscriptionId':s['id'],'resource':r.get('name'),'type':r.get('type'),'resourceGroup':r.get('resourceGroup'),'location':r.get('location'),'cost':c.get('Cost',0),'cpuAvg':m.get('cpuAvg'),'cpuMax':m.get('cpuMax'),'metricPoints':m.get('points'),'vmSize':hp.get('vmSize'),'resourceId':r.get('id')})
        if t=='microsoft.compute/disks':
            all_utilization.append({'category':'Disk','subscription':s['name'],'subscriptionId':s['id'],'resource':r.get('name'),'type':r.get('type'),'resourceGroup':r.get('resourceGroup'),'location':r.get('location'),'cost':c.get('Cost',0),'diskSizeGB':p.get('diskSizeGB'),'diskState':p.get('diskState'),'managedBy':p.get('managedBy') or '', 'resourceId':r.get('id')})
        if t=='microsoft.compute/snapshots':
            all_utilization.append({'category':'Snapshot','subscription':s['name'],'subscriptionId':s['id'],'resource':r.get('name'),'type':r.get('type'),'resourceGroup':r.get('resourceGroup'),'location':r.get('location'),'cost':c.get('Cost',0),'diskSizeGB':p.get('diskSizeGB'),'resourceId':r.get('id')})
        if t=='microsoft.network/publicipaddresses':
            ipconf=(p.get('ipConfiguration') or {}) if isinstance(p,dict) else {}
            all_utilization.append({'category':'Public IP','subscription':s['name'],'subscriptionId':s['id'],'resource':r.get('name'),'type':r.get('type'),'resourceGroup':r.get('resourceGroup'),'location':r.get('location'),'cost':c.get('Cost',0),'ipAddress':p.get('ipAddress'),'attachedTo':ipconf.get('id') or '', 'resourceId':r.get('id')})
    for c in costs:
    ...
    for a in advisor:
        ...
        meta=a.get('resourceMetadata') or {}
        rid=meta.get('resourceId') or a.get('impactedValue') or ext.get('resourceId') or ''
        ...
        if 'providers' in parts:
            ...
            if i+3 < len(parts): rname=parts[-1]
        if not rname:
            rname=meta.get('resourceName') or a.get('impactedValue') or ext.get('resourceName') or ''
        if not rtype:
            rtype=meta.get('resourceType') or a.get('impactedField') or ext.get('resourceType') or ''
        def to_float(v):
            try: return float(str(v).replace(',',''))
            except Exception: return None
        raw_amt=None; basis=''; savings=None
        if ext.get('annualSavingsAmount') is not None:
            raw_amt=to_float(ext.get('annualSavingsAmount')); basis='annualSavingsAmount'
            savings=(raw_amt/365.0*days_num) if raw_amt is not None else None
        elif ext.get('monthlySavingsAmount') is not None:
            raw_amt=to_float(ext.get('monthlySavingsAmount')); basis='monthlySavingsAmount'
            savings=(raw_amt/30.0*days_num) if raw_amt is not None else None
        elif ext.get('savingsAmount') is not None:
            raw_amt=to_float(ext.get('savingsAmount')); basis='savingsAmount'
            savings=raw_amt
        all_advisor.append({'subscription':s['name'],'subscriptionId':s['id'],'category':a.get('category'),'impact':a.get('impact'),'recommendationTypeId':a.get('recommendationTypeId'),'shortDescription':(a.get('shortDescription') or {}).get('problem') or a.get('recommendationTypeId'), 'resourceName':rname, 'resourceType':rtype, 'resourceGroup':rg, 'impactedField':a.get('impactedField'), 'impactedValue':a.get('impactedValue'), 'resourceId':rid, 'savings':savings, 'rawSavings':raw_amt, 'savingsBasis':basis, 'currency':ext.get('savingsCurrency')})
    # Candidate findings
    for r in resources:
...
data={'summary':summary,'findings':findings,'advisor':all_advisor,'resources':all_resources,'costByResource':all_cost,'costByService':all_service,'utilization':all_utilization}
...
<p><b>Estimated Advisor savings:</b> Azure Advisor may report annual, monthly, or generic savings. This report normalizes annualSavingsAmount and monthlySavingsAmount to the same {html.escape(days)}-day period as the captured cost; generic savingsAmount is shown as Azure provides it. Treat these as estimates, not guaranteed savings.</p>
...
function isMoneyCol(c){{let x=c.toLowerCase(); return x.includes('cost')||x.includes('savings')||x.includes('amount')||c=='Cost'}}
function table(title, rows, cols, limit=500){{let r=filtered(rows); let h='<div class="card"><h2>'+esc(title)+' ('+r.length+')</h2><table><thead><tr>'+cols.map(c=>'<th>'+esc(c)+'</th>').join('')+'</tr></thead><tbody>'; h+=r.slice(0,limit).map(o=>'<tr>'+cols.map(c=>'<td class="'+(c=='severity'?'sev-'+esc(o[c]):'')+'">'+(isMoneyCol(c)?money(o[c]):esc(o[c]))+'</td>').join('')+'</tr>').join(''); return h+'</tbody></table>'+(r.length>limit?'<p class="small">Showing first '+limit+' filtered rows. See CSV for full data.</p>':'')+'</div>'}}
...
function summaryCards(){{let sub=selectedSub(); let resources=bySubRows(DATA.resources,sub); let costRows=bySubRows(DATA.costByResource,sub); let advisor=bySubRows(DATA.advisor,sub); let name=sub?((DATA.summary.subscriptions.find(s=>s.id===sub)||{{}}).name||sub):'All subscriptions'; return '<div class="card"><h2>Visible Summary - '+esc(name)+'</h2><div class="grid"><div class="metric">Resources<b>'+resources.length+'</b></div><div class="metric">Cost captured<b class="money">'+money(sumCost(costRows))+'</b></div><div class="metric">Advisor recommendations<b>'+advisor.length+'</b></div><div class="metric">Est. Advisor savings for '+esc(DATA.summary.days)+' days<b class="money">'+money(sumSavings(advisor))+'</b></div></div><p><a href="#" onclick="openUtilizationSummary();return false">Open CPU / disk / utilization summary page for '+esc(name)+'</a></p><p class="small">Savings are Azure Advisor estimates normalized to the same report period, not delete-everything savings. annualSavingsAmount is prorated to '+esc(DATA.summary.days)+' days; monthlySavingsAmount is prorated to '+esc(DATA.summary.days)+' days; generic savingsAmount is shown as provided by Azure. Validate before action. The utilization link opens a separate printable page and follows the currently selected subscription.</p></div>'}}
function subscriptionBreakdown(){{let rows=DATA.summary.subscriptions.map(s=>{{let r=bySubRows(DATA.resources,s.id); let c=bySubRows(DATA.costByResource,s.id); let a=bySubRows(DATA.advisor,s.id); return {{subscription:s.name,subscriptionId:s.id,resources:r.length,cost:sumCost(c),advisor:a.length,estimatedAdvisorSavingsForPeriod:sumSavings(a)}}}}).sort((a,b)=>num(b.cost)-num(a.cost)); return table('Subscription breakdown', rows, ['subscription','subscriptionId','resources','cost','advisor','estimatedAdvisorSavingsForPeriod'])}}
function top5Spend(){{let sub=selectedSub(); let rows=bySubRows(DATA.costByResource,sub).slice().sort((a,b)=>Number(b.Cost||0)-Number(a.Cost||0)).slice(0,5); let name=sub?((DATA.summary.subscriptions.find(s=>s.id===sub)||{{}}).name||sub):'all selected subscriptions'; return table('Top 5 spend - '+name, rows, ['Cost','ResourceName','ResourceId','ResourceType','ServiceName','ResourceGroupName','subscription'])}}
function topResourcesOver100(){{let rows=DATA.costByResource.slice().filter(o=>num(o.Cost)>=100).sort((a,b)=>num(b.Cost)-num(a.Cost)); return table('Top resources by cost >= 100.00', rows, ['Cost','ResourceName','ResourceId','ResourceType','ServiceName','ResourceGroupName','subscription'], 50)}}
function topInventoryResources(){{let rows=DATA.resources.slice().sort((a,b)=>num(b.cost)-num(a.cost)); return table('Top 50 resources from inventory by captured cost', rows, ['cost','subscription','name','type','resourceGroup','location','id'], 50)}}
function simpleRows(rows, cols){{return '<table><thead><tr>'+cols.map(c=>'<th>'+esc(c)+'</th>').join('')+'</tr></thead><tbody>'+rows.map(o=>'<tr>'+cols.map(c=>'<td>'+(isMoneyCol(c)?money(o[c]):esc(o[c]))+'</td>').join('')+'</tr>').join('')+'</tbody></table>'}}
function utilSection(title, rows, cols, note){{return '<div class="card"><h2>'+esc(title)+' ('+rows.length+')</h2>'+(note?'<p class="small">'+esc(note)+'</p>':'')+(rows.length?simpleRows(rows.slice(0,20),cols):'<p>No rows for this selected subscription.</p>')+'</div>'}}
function openUtilizationSummary(){{let sub=selectedSub(); let name=sub?((DATA.summary.subscriptions.find(s=>s.id===sub)||{{}}).name||sub):'All subscriptions'; let rows=bySubRows(DATA.utilization||[],sub); let vms=rows.filter(o=>o.category==='VM CPU'); let disks=rows.filter(o=>o.category==='Disk'); let snaps=rows.filter(o=>o.category==='Snapshot'); let ips=rows.filter(o=>o.category==='Public IP'); let lowCpu=vms.slice().sort((a,b)=>num(a.cpuAvg)-num(b.cpuAvg)); let highCpu=vms.slice().sort((a,b)=>num(b.cpuAvg)-num(a.cpuAvg)); let expensiveDisks=disks.slice().sort((a,b)=>num(b.cost)-num(a.cost)); let unattachedDisks=disks.filter(o=>!val(o.managedBy)).sort((a,b)=>num(b.cost)-num(a.cost)||num(b.diskSizeGB)-num(a.diskSizeGB)); let snapshots=snaps.slice().sort((a,b)=>num(b.cost)-num(a.cost)||num(b.diskSizeGB)-num(a.diskSizeGB)); let publicIps=ips.slice().sort((a,b)=>val(a.attachedTo)?1:-1); let doc='<!doctype html><html><head><meta charset="utf-8"><title>Utilization Summary - '+esc(name)+'</title><style>body{{font-family:Segoe UI,Arial,sans-serif;margin:24px;background:#0f172a;color:#e2e8f0}}.card{{background:#111827;border:1px solid #334155;border-radius:12px;padding:16px;margin:12px 0}}table{{border-collapse:collapse;width:100%;font-size:13px}}th,td{{border-bottom:1px solid #334155;padding:6px;text-align:left;vertical-align:top}}th{{background:#1e293b}}.small{{color:#94a3b8}}.money{{color:#86efac}}</style></head><body><h1>CPU / Disk / Utilization Summary</h1><div class="card"><p><b>Scope:</b> '+esc(name)+'</p><p><b>Period:</b> last '+esc(DATA.summary.days)+' days</p><p class="small">This is a read-only summary page generated from the selected subscription filter in the main report. It shows up to 20 rows per section. CPU metrics only appear when INCLUDE_VM_METRICS=1 and Azure Monitor returned data.</p></div>'; doc+=utilSection('Lowest average CPU VMs', lowCpu, ['cost','resource','vmSize','cpuAvg','cpuMax','metricPoints','resourceGroup','subscription','resourceId'], 'Good first pass for downsize, schedule, or deallocate review. Validate workload behavior before action.'); doc+=utilSection('Highest average CPU VMs', highCpu, ['cost','resource','vmSize','cpuAvg','cpuMax','metricPoints','resourceGroup','subscription','resourceId'], 'Useful for capacity pressure or right-sizing upward review.'); doc+=utilSection('Highest cost disks', expensiveDisks, ['cost','resource','diskSizeGB','diskState','managedBy','resourceGroup','subscription','resourceId'], 'Sorts disks by captured cost where Cost Management returned resource-level disk cost.'); doc+=utilSection('Unattached / candidate disks', unattachedDisks, ['cost','resource','diskSizeGB','diskState','managedBy','resourceGroup','subscription','resourceId'], 'ManagedBy is empty. Confirm owner/snapshot policy before deleting.'); doc+=utilSection('Snapshots', snapshots, ['cost','resource','diskSizeGB','resourceGroup','subscription','resourceId'], 'Review retention. Old snapshots often accumulate storage cost.'); doc+=utilSection('Public IPs', publicIps, ['cost','resource','ipAddress','attachedTo','resourceGroup','subscription','resourceId'], 'Rows with blank attachedTo should be reviewed for deletion if unused.'); doc+='</body></html>'; let w=window.open('','_blank'); if(w){{w.document.open(); w.document.write(doc); w.document.close();}} else {{alert('Popup blocked. Allow popups for this local report, then click the utilization summary link again.')}}}}
function advisorRow(o,i){{return '<tr><td class="money">'+money(o._savings)+'</td><td>'+esc(o.savingsBasis)+'</td><td class="money">'+money(o.rawSavings)+'</td><td>'+esc(o.currency)+'</td><td>'+esc(o.impact)+'</td><td>'+esc(o.resourceName||o.impactedValue)+'</td><td>'+esc(o.resourceType||o.impactedField)+'</td><td>'+esc(o.resourceGroup)+'</td><td>'+esc(o.subscription)+'</td><td>'+esc(o.resourceId)+'</td></tr>'}}
function advisorTable(){{let rows=filtered(DATA.advisor).map((o,i)=>Object.assign({{_idx:i,_savings:num(o.savings)}},o)).sort((a,b)=>b._savings-a._savings); let groups={{}}; rows.forEach((o,i)=>{{let key=val(o.shortDescription)||'Recommendation'; if(!groups[key]) groups[key]=[]; groups[key].push(Object.assign({{_displayIdx:i}},o));}}); let ordered=Object.keys(groups).map(k=>({{key:k,rows:groups[k].sort((a,b)=>b._savings-a._savings),savings:groups[k].reduce((a,o)=>a+o._savings,0)}})).sort((a,b)=>b.savings-a.savings || b.rows.length-a.rows.length); let h='<div class="card"><h2>Advisor cost recommendations ('+rows.length+')</h2><p class="small">Grouped by recommendation description. Savings shown here are normalized estimates for the '+esc(DATA.summary.days)+'-day report period. Each group shows the first 10 affected resources; click the + / details row to expand the rest. This is read-only evidence; validate recommendations before action.</p>'; ordered.forEach(g=>{{let shown=g.rows.slice(0,10); let hidden=g.rows.slice(10); h+='<details class="card" '+(ordered.length<=3?'open':'')+'><summary><b>'+esc(g.key)+'</b> — '+g.rows.length+' resources — est. '+esc(DATA.summary.days)+'-day savings <span class="money">'+money(g.savings)+'</span></summary><table><thead><tr><th>est. period savings</th><th>source basis</th><th>raw source savings</th><th>currency</th><th>impact</th><th>resourceName</th><th>resourceType</th><th>resourceGroup</th><th>subscription</th><th>resourceId</th></tr></thead><tbody>'; h+=shown.map((o,i)=>advisorRow(o,o._displayIdx)).join(''); if(hidden.length){{h+='<tr><td colspan="10"><details><summary>+ show '+hidden.length+' more affected resources</summary><table><tbody>'+hidden.map((o,i)=>advisorRow(o,o._displayIdx)).join('')+'</tbody></table></details></td></tr>';}} h+='</tbody></table></details>';}}); return h+'</div>'}}
function cleanupRow(o){{return '<tr><td class="sev-'+esc(o.severity)+'">'+esc(o.severity)+'</td><td>'+esc(o.resource)+'</td><td>'+esc(o.type)+'</td><td>'+esc(o.resourceGroup)+'</td><td class="money">'+money(o.lastDaysCost)+'</td><td>'+esc(o.subscription)+'</td><td>'+esc(o.recommendation)+'</td><td>'+esc(o.resourceId)+'</td></tr>'}}
function cleanupSection(){{let rows=filtered(DATA.findings).sort((a,b)=>num(b.lastDaysCost)-num(a.lastDaysCost)); if(!rows.length) return '<div class="card"><h2>Rule-based cleanup findings</h2><p>No obvious abandoned-resource findings were detected for the current filter. This is separate from Advisor recommendations and high-spend optimization.</p></div>'; let groups={{}}; rows.forEach(o=>{{let key=val(o.finding)||'Finding'; if(!groups[key]) groups[key]=[]; groups[key].push(o);}}); let ordered=Object.keys(groups).map(k=>({{key:k,rows:groups[k].sort((a,b)=>num(b.lastDaysCost)-num(a.lastDaysCost)),cost:groups[k].reduce((a,o)=>a+num(o.lastDaysCost),0)}})).sort((a,b)=>b.cost-a.cost || b.rows.length-a.rows.length); let h='<div class="card"><h2>Rule-based cleanup findings ('+rows.length+')</h2><p class="small">Grouped by finding type. Each group shows the first 10 affected resources; click the + / details row to expand the rest. These are read-only candidates and should be validated with workload owners before action.</p>'; ordered.forEach(g=>{{let shown=g.rows.slice(0,10); let hidden=g.rows.slice(10); h+='<details class="card" '+(ordered.length<=3?'open':'')+'><summary><b>'+esc(g.key)+'</b> — '+g.rows.length+' resources — captured cost <span class="money">'+money(g.cost)+'</span></summary><table><thead><tr><th>severity</th><th>resource</th><th>type</th><th>resourceGroup</th><th>lastDaysCost</th><th>subscription</th><th>recommendation</th><th>resourceId</th></tr></thead><tbody>'+shown.map(cleanupRow).join(''); if(hidden.length){{h+='<tr><td colspan="8"><details><summary>+ show '+hidden.length+' more affected resources</summary><table><tbody>'+hidden.map(cleanupRow).join('')+'</tbody></table></details></td></tr>';}} h+='</tbody></table></details>';}}); return h+'</div>'}}
...
function render(){{document.getElementById('app').innerHTML = summaryCards() + subscriptionBreakdown() + top5Spend() + advisorTable() + cleanupSection() + topResourcesOver100() + table('Cost by service', DATA.costByService.slice().sort((a,b)=>Number(b.Cost||0)-Number(a.Cost||0)), ['Cost','ServiceName','subscription']) + topInventoryResources();}}
